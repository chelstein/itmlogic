...
//
// Population numbers in Part 73 filings are INFORMATIONAL ONLY — FCC
// compliance is determined by distance and field-strength tests
// (§73.207 / §73.215 / §73.333 for FM; §73.182 / §73.184 / §73.187
// for AM), not population counts.  H&D-style exhibits nonetheless
// dedicate a brief methodology page so the licensee can understand
...
export function buildPopulationMethodologySection(exhibit){
  ...
  if (isFailed){
    return {
      ...
      paragraphs: [
        ...
        'Population is informational under 47 CFR Part 73 — its absence does not affect filing readiness — but the licensee may re-run the compute with a reachable upstream if a persons-served figure is desired in the filing narrative.'
      ]
    };
  }
  ...
  const rows = [
    ...
    ['Compliance role',        'INFORMATIONAL ONLY — Part 73 compliance is field-strength-based (§73.207 / §73.215 / §73.333 FM; §73.182 / §73.184 / §73.187 AM), not population-based']
  ];
